...
def patch_vendored_target_lexicons(cargo_vendor_dir: Path) -> None:
    for crate_dir in sorted(cargo_vendor_dir.glob("target-lexicon-*")):
        if crate_dir.is_dir():
            patch_target_lexicon_riscv64a23(crate_dir)


def patch_vendored_cc_crates(cargo_vendor_dir: Path) -> None:
    for crate_dir in sorted(cargo_vendor_dir.glob("cc-*")):
        if crate_dir.is_dir():
            patch_cc_riscv64a23(crate_dir)


def patch_vendored_ahash_crates(cargo_vendor_dir: Path) -> None:
    for crate_dir in sorted(cargo_vendor_dir.glob("ahash-*")):
        if crate_dir.is_dir():
            patch_ahash_riscv64a23(crate_dir)


def patch_vendored_cargo_zigbuild_crates(cargo_vendor_dir: Path) -> None:
    for crate_dir in sorted(cargo_vendor_dir.glob("cargo-zigbuild-*")):
        if crate_dir.is_dir():
            patch_cargo_zigbuild_riscv64a23(crate_dir)


def patch_vendored_libc_crates(cargo_vendor_dir: Path) -> None:
    for crate_dir in sorted(cargo_vendor_dir.glob("libc-*")):
        if crate_dir.is_dir():
...
def audit_vendored_riscv_aliases(cargo_vendor_dir: Path, report_path: Path) -> None:
    ignored_suffixes = {
        ".a",
        ".bin",
        ".gif",
    ...
    for path in sorted(cargo_vendor_dir.rglob("*")):
        ...
        relative_path = path.relative_to(cargo_vendor_dir)
...
def main() -> int:
    ...
    if args.skip_cargo:
        ...
        patch_vendored_ahash_crates(cargo_vendor_dir)
        patch_vendored_cargo_zigbuild_crates(cargo_vendor_dir)
        patch_vendored_cc_crates(cargo_vendor_dir)
        patch_vendored_libc_crates(cargo_vendor_dir)
        patch_vendored_target_lexicons(cargo_vendor_dir)
        audit_vendored_riscv_aliases(cargo_vendor_dir, vendor_dir / "riscv64a23-audit.json")
    else:
